...
public:
	...
	double continuity(Contour& c1, Contour&c2);
private:
	std::vector<cv::Point2i> sort_contour_points(Contour& c);
	double check_symmetry(std::vector<cv::Point2i> points1, std::vector<cv::Point2i> points2);
	double check_parallelity(std::vector<cv::Point2i> points1, std::vector<cv::Point2i> points2);
};
